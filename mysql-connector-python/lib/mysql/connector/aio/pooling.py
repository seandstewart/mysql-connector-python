...
if TYPE_CHECKING:
    ...
    CMySQLConnection = MySQLConnectionAbstract

CNX_POOL_MAXSIZE = 32
CNX_POOL_MAXNAMESIZE = 64
CNX_POOL_NAMEREGEX = re.compile(r"[^a-zA-Z0-9._:\-*$#]")
ERROR_NO_CEXT = _pooling.ERROR_NO_CEXT
...
async def connect(
    *args: Any, **kwargs: Any
) -> Union[MySQLConnectionAbstract, CMySQLConnection]:
    ...
    if dns_srv:
        ...
        if "host" not in kwargs:
            kwargs["host"] = _pooling.DEFAULT_CONFIGURATION["host"]
    ...
    if "option_files" in kwargs:
        new_config = _pooling.read_option_files(**kwargs)
        return await connect(**new_config)
...
class MySQLConnectionPool:
    ...
    def __init__(
        ...
        pool_connect_timeout: Optional[int] = None,
        **kwargs: Any,
    ) -> None:
        """Constructor.
        ...
        self._pool_connect_timeout: Optional[int] = pool_connect_timeout
        self._reset_session = pool_reset_session
        self._set_pool_size(pool_size)
        self._set_pool_name(pool_name or _pooling.generate_pool_name(**kwargs))
        self._cnx_config: Dict[str, Any] = {}
        self._cnx_queue: asyncio.Queue[MySQLConnection] = asyncio.Queue(self._pool_size)
        self._pool_lock: asyncio.Lock = asyncio.Lock()
        self._config_version = uuid4()

    async def open(self) -> None:
        """Open the connection pool and fill with connections."""
        self._cnx_queue = asyncio.Queue(self._pool_size)
        self._pool_lock = asyncio.Lock()
        await self._fill_pool()

    async def _fill_pool(self, **kwargs: Any) -> None:
        if kwargs:
            await self.set_config(**kwargs)
            cnt = 0
    ...
    async def add_connection(self, cnx: Optional[MySQLConnectionAbstract] = None) -> None:
        ...
        async with self._pool_lock:
            ...
            if not cnx:
                cnx = await connect(**self._cnx_config)  # type: ignore[assignment]
                try:
                    if (
                        self._reset_session
